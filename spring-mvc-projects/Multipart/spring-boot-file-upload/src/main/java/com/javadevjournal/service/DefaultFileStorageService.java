...
public class DefaultFileStorageService implements FileStorageService {
    ...
    public FileMetaData store(MultipartFile file) throws FileStorageException {
        ...
        try {
            if (file.isEmpty()) {
                throw new FileStorageException(String.format("Failed to store empty file %s " , fileName));
            }

            Path path = Paths.get(uploadFileProperties.getUploadDir());
            Files.createDirectories(Paths.get(uploadFileProperties.getUploadDir()));
            ...
            Files.copy(file.getInputStream(), getUploadDirLocation().resolve(fileName),
